...
class Reuters_Dataset(TorchnlpDataset):
    ...
    def __init__(self, root: str, normal_class=0, tokenizer='spacy', use_tfidf_weights=False, append_sos=False,
        ...
        self.train_set0, self.test_set0 = reuters_dataset(directory=root, train=True, test=True, clean_txt=clean_txt)

        # Pre-process
        self.train_set0.columns.add('index')
        self.test_set0.columns.add('index')
        self.train_set0.columns.add('weight')
        self.test_set0.columns.add('weight')

        train_idx_normal = []  # for subsetting train_set to normal class
        train_corr = []
        self.alltrain = []
        for i, row in enumerate(self.train_set0):
            if any(label in self.normal_classes for label in row['label']) and (len(row['label']) == 1):
                self.alltrain.append(i)
        ...
        for i, row in enumerate(self.test_set0):
            if any(label in self.normal_classes for label in row['label']) and (len(row['label']) == 1):
                test_idx.append(i)
                row['label'] = torch.tensor(0)
            elif any(label in self.outlier_classes for label in row['label']) and (len(row['label']) == 1):
        ...
        self.train_set = Subset(self.train_set0, self.alltrain)
        # Subset test_set to selected normal and anomalous classes
        self.test_set = Subset(self.test_set0, test_idx)
        ...
        for row in datasets_iterator(self.train_set, self.test_set):
            # should be run only once to get the text in the test
            # row['stext'] = ""+row['text']
        ...
        # Now that the whole corpus is encoded, we can restrict it to the normal class
        self.train_idx_normal = train_idx_normal
        self.train_corr = train_corr
        self.train_set = Subset(self.train_set0, train_idx_normal)
        self.test_idx = test_idx
        for i, row in enumerate(self.train_set): row['index'] = i
        print("corplen",len(self.train_idx_normal),len(self.train_corr),len(test_idx))
